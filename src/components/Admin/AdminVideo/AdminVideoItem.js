...
export default function AdminVideoItem({ video }) {
  const [openModal, setOpenModal] = useState(false);
  const { id, title, description, url, views, duration, createdAt } = video || {};

  const handleOnClickEdit = () => setOpenModal(true);

  const [deleteVideo] = useDeleteVideoMutation();

  const handleDeleteClick = () => deleteVideo(id);
  ...
  return (
    <>
      ...
      {/* Open Modal */}
      {openModal && (
        <Modal setOpenModal={setOpenModal} title={'Edit Video'}>
          <EditVideoForm setOpenModal={setOpenModal} videoId={id} />
        </Modal>
      )}
    </>
  );
